...
class FedMTLNodePredictorClient(nn.Module):
    ...
    def local_train(self, state_dict_to_load):
        ...
        if state_dict_to_load is not None: # 加载自己的model
    ...
    def client_local_execute(device, order, **hparams_list):
       # logging.warning(f" 111FUCK list type : {type(hparams_list)}")
        ...
        client = FedMTLNodePredictorClient(client_device=device, **hparams_list)
        if order == 'train':
    ...
    def client_local_execute2(device, order, hparams_list):
        ...
        for hparams in hparams_list:
            ...
            client = FedMTLNodePredictorClient(client_device=device, **hparams) # 构造函数，此时已经将数据包含进去了
            if order == 'train':
                res = client.local_train(state_dict_to_load)
            elif order == 'val':
                res = client.local_validation(state_dict_to_load)
...
class FedMTLNodePredictor(LightningModule):
    def __init__(self, hparams, identical_agg_model, *args, **kwargs):
        ...
        self.identical_agg_model = identical_agg_model # false
    ...
    def training_step(self, batch, batch_idx):
        ...
        # fill neighbors
        for client_i, client_params in enumerate(self.client_params_list):
            # 更新每个节点，[(None,self para),(neibor1, neibor1.para),()...(neiborx, neiborx.para)]
        ...
        if self.hparams.mp_worker_num <= 1:
            ...
            for worker_i, client_params in enumerate(np.array_split(self.client_params_list, self.hparams.mp_worker_num)):
                ...
                local_train_results.append(pool.apply_async(FedMTLNodePredictorClient.client_local_execute2, args=(
        ...
        if self.hparams.mp_worker_num <= 1:
            ...
            for worker_i, client_params in enumerate(np.array_split(self.client_params_list, self.hparams.mp_worker_num)):
                ...
                local_val_results.append(pool.apply_async(
                    FedMTLNodePredictorClient.client_local_execute2, args=(
